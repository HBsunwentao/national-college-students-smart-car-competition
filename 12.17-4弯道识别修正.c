...
12.9
1.对右十字的左跳点寻找用新办法  Right_Cross_Left

2.增加对左边十字 的右边跳点的寻找办法 Left_Cross_Right

3.改良 半白十字  如：左全丢线 则判断 是否是左十字 如果是 则用 新方法     位置P470行左右
                   同理  右边全丢线也是如此处理

4.增加反馈调节机制：先针对半白 和 全白十字  。如果算法成功了则反馈为1 否则 反馈 为 0，然后
根据反馈来 选择其他十字的补线方法。 eg  470 行附近

5.修正 直十字  Strait_Cross_Right

12.10
修正 画线分母为0 的情况 实际上不是问题
发现半白 十字 也分为 1.直道半白十字  2.左右十字半百
改进：1.将卡口拆成左右 部分  
      2.将半边全白 十字的补法进行修正。
	  3.将全白十字 增加反馈变量
未来改进区间：将搜线 也分成 左右 并且 又 八向 和 二向的区别。将十字分成四个部分单独 弄情况

12.12
添加 负反馈调节 int  Right_Cross_Right   int Left_Cross_Left
修正Mid_Deep 到顶全部都是白的情况
未来改进区间：左右十字判别方法：需要改进

12.15
修正对远十字的判别：判别错误的原因是：当很远的时候 Left_Lost_Flag2 即十字跳跃式跳点失效
因为远处距离变小  原本为近处的设计临界不再适用：解决措施是 设计动态阈值，阈值大小直接和 
图像的远近有关

远十字的处理：当很远的时候 直接用中线卡口

新问题：由于远十字存在的原因导致边线搜索失败 停留在十字区域内部
       简单的修正方法：先 定中点搜，搜不下去 后 再用上一行 中点 来搜
	   
发现新问题：判断弯道的掉线flag 存在非全白掉线的状况  而且不能简单的 更改 掉线Left_Lost_Flag的判断条件
*/
...
//控制舵机使用的变量  
  ...
  int Servo_Control2=160;             //控制舵机用的 中点值 之前
    int Bend_Flag=0;        //弯道flag
  
  int In_Bend_Flag=0;          //入弯道flag 12.17
  int Out_Bend_Flag=0;         //出弯道flag
...
int  Right_Cross_Right(unsigned char buff[]);  //启发条件：十字非常向右，且右边 掉线
void Right_Cross_Left(unsigned char buff[]);   //          右十字的左边 补图画线
int  Left_Cross_Left(unsigned char buff[]);    // 启发条件：十字非常向左，且左边 掉线
void Left_Cross_Right(unsigned char buff[]);   //            左边十字的右边
int  Right_Cross_All_White(unsigned char buff[]);  //十字右下边 全白的情况
int  Left_All_White(unsigned char buff[]);   // 十字左下边全白的情况
...
int All_White(unsigned char buff[]);            //左右全白新算法
void Variable_Clear(void);                        //变量初始化
void Servo_Control(void);                         //舵机偏转控制程序
int All_White_Right(unsigned char buff[]);         //卡口 右边
int All_White_Left(unsigned char buff[]);           // 卡口左边   
int Long_Cross_Strait(unsigned char buff[]);        //长直赛道的十字
void LineSearch2(unsigned char buff[]);              //长十字边界改良法
//...........边界平滑处理..............//
//未完成
				 Variable_Clear();                  //变量清零
...
//..........................特殊图像的提前判断............................//
  ...
  for(i=BOTTOM;i>=TOP;i--)
	  ...
	  if(Mid_Deep<100)LineSearch(imgbuff);
		else LineSearch2(imgbuff);//边界搜索
  ...
  {
	  //...........记录左丢线
	  if( ((lLine[i-1]==LEFTLINE) ||((lLine[i]-lLine[i-1])>50)) && !Left_Lost_Flag){Left_Lost_Flag=1;Left_Lost_Start=i;}
	  if(lLine[i]==LEFTLINE && lLine[i-1]!=LEFTLINE && Left_Lost_Flag){Left_Lost_End=i;}
	  //...........记录右丢线
	  if( ((rLine[i-1]==RIGHTLINE) ||((rLine[i-1]-rLine[i])>50)) && !Right_Lost_Flag){Right_Lost_Flag=1;Right_Lost_Start=i;}
...
void LineSearch2(unsigned char buff[])
{
	int i,j,l,r;
	int Mid=160;
	for(i=BOTTOM;i>BOTTOM-Mid_Deep;i--)
	{
		l=0;
		r=319;
		lLine[i]=0;
	...
	Mid=(rLine[i+1]+lLine[i+1])/2;
	lSum=BOTTOM-Mid_Deep+1;
	for(i=BOTTOM-Mid_Deep;i>=TOP;i--)
	{
		lLine[i]=0;
		rLine[i]=319;
		for(j=Mid;j>=LEFTLINE;j--)
...
/*
如果是 远距离的 十字 一般是直十字  MidDeep 足够
卡口法找到上面的口 确定二点
然后让下面依据斜率往上补边界 一直补到 卡口法找到的 上界
*/
int Long_Cross_Strait(unsigned char buff[])
{
	//卡口找点 ,因为是远十字，所以是从第 60 行开始 ：要求外部事先判断好
	int i,j;
	int mid=160;
	int l=0,r=319;
...
int Left_Cross_Left(unsigned char buff[])
{
	...
	if(t==2)
	{
		DrawLine2(buff,ex[0],ey[0],ex[3],ey[3]);  //在原图上补全十字
		return 1;
	}
	return 0;
}
...
void Left_Cross_Right(unsigned char buff[])
{
	...
	if(t==1){		//当找到了下面的点  就开始寻找上面的跳点
		...
		if((buff[i*40+(ey[1]+1)/8]&pow[7-(ey[1]+1)%8])!=0)  //先找到上跳点所在的 线在哪一行
		{
			partS=i+1;
			break;
		}
		
	for(i=partS;i<=BOTTOM;i++)  //从上往下
	{
		for(j=ey[1];j>=LEFTLINE;j--)  
		if(((buff[i*40+j/8]&pow[7-j%8])!=0)||(j==LEFTLINE))  //通过右边边界距离来寻找跳点
		{
			nRight=j;
			break;
		}
...
int Right_Cross_Right(unsigned char buff[])  //启发条件：十字非常向右，且右边 掉线
{
	...
	if(t==2)
	{
		DrawLine2(buff,ex[1],ey[1],ex[2],ey[2]);  //在原图上补全十字
		return 1;
	}
...
for(i=lLine[BOTTOM];i<=RIGHTLINE;i++) //寻找 左下点   竖着搜跳点
	...
	if(t==1){		//当找到了下面的点  就开始寻找上面的跳点
		...
		if((buff[i*40+(ey[0]+1)/8]&pow[7-(ey[0]+1)%8])!=0)  //先找到上跳点所在的 线在哪一行
		{
			partS=i+1;
			break;
		}
	...
	for(i=partS;i<=BOTTOM;i++)  //从上往下
	{
		...
		if(((buff[i*40+j/8]&pow[7-j%8])!=0)||(j==RIGHTLINE))  //通过右边边界距离来寻找跳点
		{
			nLeft=j;
			break;
		}
...
int All_White(unsigned char buff[]) //当左右两边 均为空白全丢线：直接中线长度固定来卡十字
{
	str.Format("全白卡口\n\r");PrintDebug(str);
	int i,j;
	int l=0,r=319,l1,r1;
	...
	{
		if((buff[i*40+160/8]&pow[7-160%8])!=0)return 0;
        l=0;r=319;
		...
		if(r-l<=160 && r<260 && l>60)   //当找到正确了方向就准备进行补线
		{
			...
			Mid=(r+l)/2;
		    for(j=Mid;j<=RIGHTLINE;j++)
			if((buff[(i-10)*40+j/8]&pow[7-j%8])!=0)
			{
				r1=j;
				//str.Format("r1=%d\n\r",r1);PrintDebug(str);
				break;
		   ...
		   for(j=Mid;j>=LEFTLINE;j--)
		    ...
		    for(j=Mid;j<=RIGHTLINE;j++)
			if((buff[(i-2)*40+j/8]&pow[7-j%8])!=0)
			{
				r=j;
				//str.Format("r1=%d\n\r",r1);PrintDebug(str);
				break;
		   ...
		   for(j=Mid;j>=LEFTLINE;j--)
			...
			K=(l-l1)*1.0/8;
			B=l-K*i;
			DrawLine3(buff,i-10,l1,K,B);
			
			K=(r-r1)*1.0/8;
			B=r-K*i;
			DrawLine3(buff,i-10,r1,K,B);
...
//=================
//功能：当为 直到十字 （弯曲度很小） 而且 右边全白 的情况
//  	此时  左边已经补线完全  所以可以根据左边的边界 向右 卡口 而不是直接用中线卡口
//==================
int All_White_Right(unsigned char buff[])   
{
	...
	float K,B;
	int Mid;
	LineSearch(buff);  //因为已经补好左边的线 所有直接 搜一次边线
	for(i=BOTTOM;i>lSum;i--)
	{
		
		//str.Format("左=%d 右=%d 右-左=%d\n\r",lLine[i],rLine[i],rLine[i]-lLine[i]);PrintDebug(str);
		if((rLine[i]-lLine[i])<=85 && rLine[i]<260 && lLine[i]>60)   //当找到正确了方向就准备进行补线
		{
           //str.Format("成功进入！\n\r");PrintDebug(str);
...
//=================
//功能：当为 直到十字 （弯曲度很小） 而且 左边全白 的情况
//  	此时  右边已经补线完全  所以可以根据右边的边界 向左 卡口 而不是直接用中线卡口
//==================
int All_White_Left(unsigned char buff[])   
{
	...
	float K,B;
	int Mid;
	LineSearch(buff);  //因为已经补好右边的线 所有直接 搜一次边线
	for(i=BOTTOM;i>lSum;i--)
	{
...
void Servo_Control(void)
{
	...
	Servo_Control1/=num;
		...
		//简单判断入弯和出弯 
	if(((Servo_Control1-Servo_Control2)>3 && Servo_Control1>0)||((Servo_Control1-Servo_Control2)<-3 && Servo_Control1<0))
	{
		In_Bend_Flag=1;
	}
	if(((Servo_Control1-Servo_Control2)>3 && Servo_Control1<0)||((Servo_Control1-Servo_Control2)<-3 && Servo_Control1>0))
...
void Variable_Clear(void)
{
    ...
    //Servo_Control2=160;             //控制舵机用的 中点值 之前
		 Bend_Flag=0;        //弯道flag
	 In_Bend_Flag=0;          //入弯道flag
     Out_Bend_Flag=0;         //出弯道flag
}

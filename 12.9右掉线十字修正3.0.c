...
12.9
对右十字的左跳点寻找用新办法  Right_Cross_Left

增加对左边十字 的右边跳点的寻找办法 Left_Cross_Right

改良 半白十字  如：左全丢线 则判断 是否是左十字 如果是 则用 新方法     位置P470行左右
                   同理  右边全丢线也是如此处理

增加反馈调节机制：先针对半白 和 全白十字  。如果算法成功了则反馈为1 否则 反馈 为 0，然后

 ...
 if(Cross_Flag)  // 十字识别 的启发条件
  {
	...
	{	
		if(Left_Lost_Flag && Left_Lost_Start==BOTTOM)
		{
			...
			{
				if(Left_Cross_Flag)Left_Cross_Right(imgbuff);
				else Strait_Cross_Right(imgbuff);
			}
			else 
				if(Strait_Cross_Flag)All_White(imgbuff);
		...
		if(Right_Lost_Flag && Right_Lost_Start==BOTTOM)
		{
			...
			{
				if(Right_Cross_Flag)Right_Cross_Left(imgbuff);
				else Strait_Cross_Left(imgbuff);
			}
			else
				if(Strait_Cross_Flag)All_White(imgbuff);

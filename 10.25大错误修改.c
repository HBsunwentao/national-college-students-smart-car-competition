...
//.............特殊图像判断 所用的变量
  ...
  int Left_Lost_Flag2=0,Right_Lost_Flag2=0;   //跳跃式掉线，专注于十字的掉线
 ...
 if(Cross_Flag)  // 十字识别 的启发条件
  {
	...
	//if(All_White_Flag)All_White(imgbuff);
...
void All_White(unsigned char buff[])
{
	int i,j;
	int l=0,r=319,l1,r1;
	float K,B;
	...
	{ 
		...
		if(r-j<=160 && r<260 && l>60)
		{
		   for(j=160;j<=RIGHTLINE;j++)
			if(buff[(i+4)*40+j/8]&pow[7-j%8]!=0)
			{
				l1=j;
				str.Format("l1=%d\n\r",l1);PrintDebug(str);
				break;
		   ...
		   for(j=160;j>=LEFTLINE;j--)
			...
			K=(l-l1)*1.0/4;
			B=i-K*i;
			DrawLine3(buff,i,l,K,B);
			
			K=(r-r1)*1.0/4;
			B=i-K*i;
			DrawLine3(buff,i,r,K,B);

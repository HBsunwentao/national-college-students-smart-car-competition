...
void Servo_Control(void)
{
	...
	Servo_Control1=0;
	for(i=VIEWROWDOWN;i>=VIEWROWUP;i--)
	{
		Servo_Control1+=MidLine[i];
		++num;
		if(i>=lSum2)break;
	}
	Servo_Control1/=num;
  ...
  if(Right_Continuous_Flag==0 || Left_Continuous_Flag==0)
  {
	  Servo_Strait_Flag=0;
  }
}
...
void Variable_Clear(void)
{
     ...
     Left_Continuous_Flag=1;Right_Continuous_Flag=1;Left_Continuous_Delta=0;Right_Continuous_Delta=0;
      Left_Uncontinuous_Flag=0;Right_Uncontinuous_Flag=0;

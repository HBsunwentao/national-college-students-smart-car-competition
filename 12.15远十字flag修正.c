...
//..........................特殊图像的提前判断............................//
  ...
  for(i=BOTTOM;i>lSum+3;i--)
  {
      ...
      //十字跳跃形式掉线 的判断（修正版）
	  if(((lLine[i]-lLine[i-1])>=1.29*i) && !Left_Lost_Flag2 && i<60)
	  {
		  str.Format("发现左边跳\n\r");PrintDebug(str);
		  Left_Lost_Flag2=1;
		  Left_Lost_Start2=i;
	  }
	  if(Left_Lost_Flag2 && ((lLine[i-1]-lLine[i])>1.29*i)&& i<60 )
	  {
		  Left_Lost_End2=i;
	  }
	  if(((rLine[i-1]-rLine[i])>=1.29*i) && !Right_Lost_Flag2)
	  {
		  str.Format("发现右边跳\n\r");PrintDebug(str);
		  Right_Lost_Flag2=1;
		  Right_Lost_Start2=i;
	  }
	  if(Right_Lost_Flag2 && ((rLine[i]-rLine[i-1])>1.29*i)  )
	  {
		  Right_Lost_End2=i;
	  }
  }
 ...
 // if(Right_Uncontinuous_Flag && !Right_Straint_Flag)Cross_Flag=0;
  ...
  //超远十字修正
  if(Left_Lost_Flag2 && Right_Lost_Flag2)Cross_Flag=1;

...
直道十字单边正确 另一边失败的问题 ：仿造单边卡口法的思路

12.17
超右十字 近上顶口十字判断
新flag： 如果是右十字+弯道=超右十字十字弯道==》则 右边 二个 口 则 直接当作弯道处理
两个flag：1.之前判断失效 2. 前三十行 向左 或者向右  3.  左边 或者 右边有二个开口

//简单判断入弯和出弯 用偏差的 一阶导数 和 方向来确定入弯和出弯
*/
...
int pow2[8]={1,2,4,8,16,32,64,128};                    //个人全局变量开始处
...
//.............特殊图像判断 所用的变量
  ...
  int Left_Lost_Flag2=0,Right_Lost_Flag2=0;   //跳跃式掉线，专注于十字的掉线 即明显的 边界坑洞
	...
	int Left_Lost_Num=0,Right_Lost_Num=0;     //数左右缺口数目 配合 前30行代码判断十字弯 上顶口的情况
	int Left_Lost_End4=0,Right_Lost_End4=0;   //十字弯的 上弯边界
	int Bend=0;                               //前三十行偏差
    ...
    if((imgbuff[i*40+MidLine[i]/8]&pow2[7-MidLine[i]%8])==0)
			{
				...
				imgbuff[i*40+MidLine[i]/8]+=pow2[7-MidLine[i]%8];   //显示中线    通过加法来显示中线  化0为 一
		    }
  }
...
void LineSearch(unsigned char buff[])
{
      ...
      for(i=BOTTOM;i>=TOP;i--)              //从下面向上面搜索
	  {
		  ...
		  for(j=Mid;j<=RIGHTLINE;j++)
			  if((buff[i*40+j/8]&pow2[7-j%8])!=0)   //从中间向两边寻找 边线  1为黑边
			  {
				  rLine[i]=j-1;                        //记录边线位置
				  break;
			  }
...
for(i=lLine[BOTTOM];i<=RIGHTLINE;i++) //寻找 左下点   竖着搜跳点
 ...
 Mid=ex[0]-10;if(Mid<TOP)Mid=TOP;
	...
	Mid=nUp+10;if(Mid>BOTTOM)Mid=BOTTOM;
    	  if((buff[Mid*40+i/8]&pow2[7-i%8])!=0)break;            // 保险装置 ，防止搜进边界里面去了  对于 解决 误判十字 有80%的作用 
	}
...
void Left_Cross_Right(unsigned char buff[])
{
	...
	if(t==1){		//当找到了下面的点  就开始寻找上面的跳点
		...
		if((buff[i*40+(ey[1]+1)/8]&pow2[7-(ey[1]+1)%8])!=0)  //先找到上跳点所在的 线在哪一行
		{
			partS=i+1;
			break;
		}
	...
	for(i=partS;i<=BOTTOM;i++)  //从上往下
	{
		...
		if(((buff[i*40+j/8]&pow2[7-j%8])!=0)||(j==LEFTLINE))  //通过右边边界距离来寻找跳点
		{
			nRight=j;
			break;
		}
...
for(i=lLine[BOTTOM];i<=RIGHTLINE;i++) //寻找 左下点   竖着搜跳点
	...
	if(t==1){		//当找到了下面的点  就开始寻找上面的跳点
		...
		if((buff[i*40+(ey[0]+1)/8]&pow2[7-(ey[0]+1)%8])!=0)  //先找到上跳点所在的 线在哪一行
		{
			partS=i+1;
			break;
		}
	...
	for(i=partS;i<=BOTTOM;i++)  //从上往下
	{
		...
		if(((buff[i*40+j/8]&pow2[7-j%8])!=0)||(j==RIGHTLINE))  //通过右边边界距离来寻找跳点
		{
			nLeft=j;
			break;
		}
...
void Variable_Clear(void)
{
	 ...
	 Left_Lost_Num=0;Right_Lost_Num=0;     //数左右缺口数目 配合 前30行代码判断十字弯 上顶口的情况
	  Left_Lost_End4=0;Right_Lost_End4=0;   //十字弯的 上弯边界
